...
const isStaging = ((!isDev && window.ABC && ABC.appId) || '').indexOf('News') === -1; // ABC.appId is undefined in Preview sites.
const root = document.querySelector(`[data-interactive-private-health-root]`);
const api = new Client(`interactive-private-health${isDev ? '_dev' : isStaging ? '_staging' : ''}`);
let content = {};

function init() {
  const App = require('./components/App');
  render(<App content={content} api={api} isDev={process.env.NODE_ENV === 'development'} />, root, root.firstChild);
}
